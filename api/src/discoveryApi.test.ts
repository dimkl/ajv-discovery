...
import { createRequiredError, createTypeError } from "../testHelpers";
...
describe("DiscoveryApi", () => {
    ...
    describe("validateSchema(schema, dataAccessor, dataSetter, allowBulkData)", () => {
        ...
        test("supports bulk data as array by default", async () => {
            const ajv = DiscoveryApiFactory.createAjv();
            const discoveryApi = new DiscoveryApi(ajv);

            const data = [
                { username: "userA", additional: "a" },
                { username: "userB", additional: "b" },
            ];
            const dataAccessorSpy = () => data;
            const dataSetterSpy = jest.fn();
            ...
            expect(dataSetterSpy).toBeCalledTimes(1);
            expect(dataSetterSpy).toBeCalledWith(
        ...
        test("fails bulk data as array when allowBulkData is false", async () => {
            const ajv = DiscoveryApiFactory.createAjv();
            const discoveryApi = new DiscoveryApi(ajv);
            ...
            await expect(
                ...
                errors: expect.arrayContaining([createTypeError()]),
            });
